...
################################################# END SECTION OF ANALYSIS FOR scRNA-Seq DATA #################################################
    ...
    if 'sensitive' in adata.obs.columns:
        ...
        raise KeyError("'sensitive' 列不存在于 adata.obs 中")
    ...
    fpr, tpr, _ = roc_curve(Y_test, lb_results)
    roc_auc = auc(fpr, tpr)


    precision, recall, _ = precision_recall_curve(Y_test, lb_results)
    pr_auc = auc(recall, precision)


    from sklearn.metrics import roc_curve

    fpr, tpr, thresholds = roc_curve(Y_test, lb_results)
    ...
    df.to_csv('save/data1-sc-ROC_data.csv', index=False)

    from sklearn.metrics import precision_recall_curve


    precision, recall, thresholds = precision_recall_curve(Y_test, lb_results)
    ...
    df.to_csv('save/data1-sc-PR_data.csv', index=False)
    ...
    file = 'save/ori_result/bulk_f' + data_name + '_f1_score_roc_auc_ori.txt'
    with open(file, 'a+') as f:
        f.write(para + '\t' + str(f1score) + '\t' + str(roc_auc) + '\n')
